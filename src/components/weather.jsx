...
const Weather = () => {
  ...
  return (
    <div
      className="flex items-center justify-center h-screen bg-center bg-no-repeat bg-cover font-poppins py-16"
      style={{ backgroundImage: `url(${weather})` }}
      >
      <div className="flex flex-col items-center justify-center h-screen w-full bg-[#07754E] opacity-90">
          <h1 className="text-lg lg:text-4xl md:text-2xl font-bold relative bottom-52 text-white text-center mb-[-15px]">
          Cuaca Hari Ini di Kota-Kota Jawa Timur
          </h1>
        <select
          value={city}
          onChange={handleCityChange}
          className="relative bottom-40 p-2 mb-20 lg:mb-16 rounded-full shadow-lg text-center text-white border-0 focus:outline-none bg-white/30 backdrop-blur-md mt-3">
          {cities.map((cityName) => (
        ...
        {weatherData && (
          <div
            className={`absolute bg-white/35 p-8 rounded-lg shadow-lg text-center mt-10 backdrop-blur-md transition-transform duration-500 ${
              isShiftedRight ? "translate-x-80" : "translate-x-0"
            }`} onClick={handleBackdropClick}>
            <h2 className="text-3xl mb-4 text-white">
              {weatherData.name}
            </h2>
            <h3 className="text-5xl font-semibold text-[#FFF100] mb-4">
